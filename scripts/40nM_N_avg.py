...
#40nM_N_df = Ben_data_import.loc[:'Time(days)','40nM - A.1':'40nM - C.2']  #df of times and the 40nm added data

40nM_N_data_df = Ben_data_import.loc [:,'40nM - A.1':'40nM - C.2']    #making a df of just the data so I can average the values

40nM_N_data_avg_df = 40nM_data_data_df.mean(axis=1)  #making a dataframe of just the mean column values (axis = 1 means that I am averaging each column value along each row so as to give an average column) axis = 0 would give a row of avereavereges from each column instead.

40nM_N_avg = array((Ben_data_import['40nM - A.1'])) #turining 40nM_N_A1 into an array

mean_data = 40nM_data_data_df.mean(axis=1)
