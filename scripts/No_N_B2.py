...
No_N_B2 = Ben_data_import ['NoN-B.2']      #nameing the technical replicate Producer cell density values as No_N_B2
No_N_B2 = array((Ben_data_import['NoN-B.2'])) #turining No_N_B2 into an array
Ps = No_N_B2 #Cells per mL of Pro from data array; renaming it Ps
...
Rs = r_[[((((max(No_N_B2))-No_N_B2[0])*(Q)),0)]] #making an array for Rs values, just the amount of R calculated from taking the peak cell growth and finding change in cell density and finding resource needed for that cell increase based on cell Nitrogen quota
...
No_N_B2 = array((Ben_data_import['NoN-B.2']))
Ps = No_N_B2
Rs = r_[[((((max(No_N_B2))-No_N_B2[0])*(Q)),0)]]     #should the endpoint be somthing other than 0? 


# initial conditions
  #inits = r_[[Rs, Ps]]
inits = r_[[(((max(No_N_B2))-No_N_B2[0])*(Q)), No_N_B2[0]]]     #what initial conditions do I put here??? For R and P? 
...
#P = No_N_B2[0]   #makes the starting value of P be the first data point collected for P in Excel data file
...
for it in arange(1,nits,1):  #making an evenly spaces arrangement of values from 1 to nits (number of itterations) on the step scale of 1.  
        ...
        R, P = (((max(No_N_B2))-No_N_B2[0])*(Q)), No_N_B2[0]  #have to reassign initial values for R and P because we are now opperating in a loop
...
R,P = ((((max(No_N_B2))-No_N_B2[0])*(Q)), No_N_B2[0])
